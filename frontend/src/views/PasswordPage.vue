<template>
  <!-- Conteneur principal -->
  <div>
    <!-- Arrière-plan avec une image du Louvre -->
    <div class="background"></div>
    <div class="container">
      <!-- Checkbox pour activer/désactiver le formulaire -->
      <input type="checkbox" id="check">

      <!-- Formulaire de récupération de mot de passe -->
      <div class="forgot form">
        <header>Mot de passe oublié</header>
        <form action="#">
          <!-- Champ pour l'email -->
          <div class="input-container">
            <img src="../assets/icon_mail.png" alt="Icône email">
            <input type="text" placeholder="Entrez votre email">
          </div>

          <!-- Bouton pour envoyer l'email de récupération -->
          <input type="button" class="button" id="send" value="Envoyez un email">
        </form>

        <!-- Lien pour se connecter -->
        <div class="login">
          <span class="login">Tu souhaites te connecter ?
            <label for="check"><router-link to="/login">Se connecter</router-link></label>
          </span>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
export default {
  // Composant sans données ou méthodes spécifiques
};
</script>

<style scoped>
/* Styles CSS spécifiques à ce composant */

/* Arrière-plan avec une image du Louvre */
.background {
  background-image: url('../assets/Louvre_Cour_Carree.jpg');
  background-size: cover;
  background-repeat: no-repeat;
  background-attachment: fixed;
  position: fixed;
  top: 0;
  left: 300;
  width: 100%;
  height: 100%;
  z-index: -1;
  /* Place l'arrière-plan derrière les autres éléments */
}

/* Styles pour les conteneurs d'entrée */
.input-container {
  display: flex;
  /* Utilisez flexbox pour aligner horizontalement */
  justify-content: center;
  /* Centre les éléments horizontalement */
  background-color: #dcb253;
  height: 60px;
  margin-bottom: 20px;
  border-radius: 10px;
}

.input-container img {
  /* Styles pour les images dans les champs */
  margin-top: 15px;
  margin-right: 8px;
  margin-left: 10px;
  width: 30px;
  height: 30px;
  color: white;
}

/* Styles globaux */
* {
  margin: 0;
  padding: 0;
  box-sizing: border-box;
  font-family: 'Poppins', sans-serif;
}

body {
  /* Styles pour le corps du document */
  min-height: 100vh;
  width: 100%;
  background: #009579;
}

.container {
  /* Styles pour le conteneur principal */
  position: absolute;
  top: 50%;
  left: 50%;
  transform: translate(-50%, -50%);
  max-width: 430px;
  width: 100%;
  background: #004225;
  border-radius: 7px;
  box-shadow: 0 5px 10px rgba(0, 0, 0, 0.3);
}

.container .registration {
  display: none;
  /* Cache le formulaire d'inscription par défaut */
}

#check:checked~.registration {
  display: block;
  /* Affiche le formulaire d'inscription si la checkbox est cochée */
}

#check:checked~.forgot {
  display: none;
  /* Cache le formulaire de récupération si la checkbox est cochée */
}

#check {
  display: none;
  /* Cache la checkbox par défaut */
}

.container .form {
  /* Styles pour le formulaire */
  padding: 2rem;
}

.form header {
  /* Styles pour l'en-tête du formulaire */
  font-size: 2rem;
  font-weight: 500;
  text-align: center;
  margin-bottom: 1.5rem;
  color: white;
}

.form input {
  /* Styles pour les champs de saisie */
  height: 60px;
  width: 90%;
  padding: 0 15px;
  font-size: 17px;
  margin-bottom: 1.3rem;
  border: 1px solid #ddd;
  border-radius: 6px;
  border-top-left-radius: 0;
  border-bottom-left-radius: 0;
  outline: none;
}

#send {
  /* Styles pour le bouton d'envoi */
  border-top-left-radius: 6px;
  border-bottom-left-radius: 6px;
}

.form input:focus {
  /* Styles pour le champ de saisie lorsqu'il est en focus */
  box-shadow: 0 1px 0 rgba(0, 0, 0, 0.2);
}

.form a {
  /* Styles pour les liens dans le formulaire */
  font-size: 16px;
  color: #dcb253;
  text-decoration: none;
}

.form a:hover {
  /* Styles pour les liens survolés */
  text-decoration: underline;
}

.form input.button {
  /* Styles pour le bouton du formulaire */
  width: 100%;
  margin-left: 0px;
  color: #000000;
  border: none;
  background: #dcb253;
  font-size: 1.2rem;
  font-weight: 500;
  letter-spacing: 1px;
  margin-top: 15px;
  cursor: pointer;
  transition: 0.4s;
}

.form input.button:hover {
  /* Styles pour le bouton du formulaire au survol */
  background: #ffde92;
}

.login {
  /* Styles pour la section "Tu souhaites te connecter ?" */
  font-size: 17px;
  text-align: center;
  color: white;
}

.login label {
  /* Styles pour le lien "Se connecter" dans la section "Tu souhaites te connecter ?" */
  color: #dcb253;
  cursor: pointer;
}

.login label:hover {
  /* Styles pour le lien "Se connecter" survolé */
  text-decoration: underline;
}</style>
